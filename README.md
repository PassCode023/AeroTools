...
| 平台 | 状态 | 说明 |
|---|---|---|
| 微信小程序 | ✅ 可构建 | `npm run build:mp-weixin`，主包约 710KB |
...
- **机场查询**：已收录 **553 家**机场（境内运输机场 **270/270** 与民航局名录全量对账，含 33 起官方确认更名与 34 家补齐；另含港澳台 10 条与全球主要机场），IATA/ICAO/中文机场名/中文城市名实时搜索，详情含曾用名/坐标/时区/海拔与数据来源、资料截至日期
...
- **海拔字段完整率约 13%**：全球多数机场缺海拔，缺失统一显示"—"；完整率统计记录在 `dbversion.json`，应用界面不展示
...
- APP 打包（APK/IPA）需 HBuilderX 与相应开发者账号，本仓库交付源码与配置；iOS 真机验收需 Apple 开发者账号（未就绪时按迭代计划降级路径执行）
