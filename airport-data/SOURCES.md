...
### 海拔回填批次（2026-09-14，工具脚本 `tools/backfill-elev.js`）

- 回填 **435 条**缺失海拔：ourairports 422 条（主源，Unlicense）＋airportsdata 13 条（OA 缺位回落，逐条人工地理常识复核）。海拔完整率 14% → **93%**。
- 剔除 28 条上游 0 值（占位非实测，如霍林郭勒）；剔除重庆巫山 1680m（与公开标高 1774.8m 矛盾且 OA 缺位，保持为空）。
- **连带修正图木舒克唐王城机场**：库内旧 IATA `TMC` 实为印尼 Tambolaka 机场的码（双源一致），系假性撞车并曾被对账工具拼入 49m 假海拔；按 AD 载明改为 `TWC`、补 `icao ZWTS`、坐标精确化、海拔 1090m。
- 剩余队列：29 条（28 条 0 值占位＋巫山），维持留空待官方 AIP。

### 坐标精度升级批次（2026-09-14，工具脚本 `tools/precision-coords.js`）
